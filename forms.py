...
class BudgetForm(FlaskForm):
    ...
    # Financial History
    short_term_savings = DecimalField("How much money do you have saved for expenses you'll face within the next year? (e.g., groceries, textbooks, or travel)", validators=[InputRequired(), NumberRange(min=0)])
    long_term_savings = DecimalField("How much money do you have saved for goals more than a year away? (e.g., savings for post-graduation plans, a car, or future investments)", validators=[InputRequired(), NumberRange(min=0)])
    investments = DecimalField("How much money do you currently have invested? (e.g., in stocks, bonds, or mutual funds)", validators=[InputRequired(), NumberRange(min=0)])
    ...
    debt_payments = DecimalField('How much is your monthly payment for debt (credit cards, loans, etc.)?', validators=[InputRequired(), NumberRange(min=0)])
